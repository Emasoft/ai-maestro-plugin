...
Modes:
  --gate                  Pre-push gate: orchestrator check + lint + validate + tests
                          only (no bump/push).
  --install-hook          Activate the static committed .githooks/pre-push hook by
                          setting core.hooksPath = .githooks (fresh-clone activation).
...
# Kept as module constants so the gate, the helper, and the test all agree on
# one source of truth (TRDD-71a2239a). The pin format is `sha256sum`'s own
# (`<hex>␠␠<relpath>`) so `sha256sum -c .githooks/pre-push.sha256` (run from
# the repo root) verifies it identically in CI and locally.
...
def check_pre_push_hook_integrity(root: Path) -> tuple[bool, str]:
    """Read-only check that the on-disk pre-push hook matches its sha256 pin.

    The pre-push hook is a STATIC committed file (NOT generated by publish.py).
    Pinning its sha256 in `.githooks/pre-push.sha256` makes an out-of-band edit
    that silently weakens the hook impossible to merge unseen: editing the hook
    forces a matching pin update in the SAME commit (CI re-verifies the pin —
    TRDD-71a2239a). This function NEVER writes/rewrites the hook or the pin; it
    only recomputes the hash and compares, so the publish gate aborts (rather
    than rubber-stamps) when the two have drifted apart locally — before push,
    ahead of CI.

    The pin file uses `sha256sum`'s native format (`<hex>␠␠<relpath>`), so only
    the leading hex digest is compared; the trailing path is informational.
    Returns (ok, message): ok=True with an empty message when they match, else
    ok=False with a reviewer-facing explanation. Missing hook OR missing pin is
    a hard FAIL (a removed pin is exactly the silent-weakening this prevents).
    """
    hook_path = root / PRE_PUSH_HOOK_REL
    pin_path = root / PRE_PUSH_HOOK_PIN_REL
    if not hook_path.is_file():
        return False, f"{PRE_PUSH_HOOK_REL} is missing — cannot verify hook integrity."
    if not pin_path.is_file():
        return False, (
            f"{PRE_PUSH_HOOK_PIN_REL} is missing — the pre-push hook is unpinned. "
            f"Regenerate it with: sha256sum {PRE_PUSH_HOOK_REL} > {PRE_PUSH_HOOK_PIN_REL}"
        )
    actual = hashlib.sha256(hook_path.read_bytes()).hexdigest()
    # The pin is `<hex>  <relpath>`; take the first whitespace-delimited token.
    pin_first = pin_path.read_text(encoding="utf-8").strip().split()
    expected = pin_first[0] if pin_first else ""
    if actual != expected:
        return False, (
            f"{PRE_PUSH_HOOK_REL} changed but its .sha256 pin wasn't updated — "
            f"edit + re-pin in the same commit. Expected {expected or '(empty)'}, "
            f"got {actual}. Re-pin with: sha256sum {PRE_PUSH_HOOK_REL} > {PRE_PUSH_HOOK_PIN_REL}"
        )
    return True, ""
...
def install_hook(root: Path) -> int:
    """Activate the static committed pre-push hook for this clone.

    The hook is a STATIC, git-tracked file at ``.githooks/pre-push`` (NOT
    generated by publish.py — see that file's header and TRDD-4b298890). Git
    runs hooks directly out of the directory named by ``core.hooksPath``, so
    activation is simply pointing ``core.hooksPath`` at ``.githooks`` — there
    is nothing to copy into ``.git/hooks/``. This is the fresh-clone story:
    after ``git clone`` a developer runs ``uv run python scripts/publish.py
    --install-hook`` once to arm the gate. Idempotent: re-running it when the
    config already points at ``.githooks`` is a no-op.
    """
    cprint(f"\n{BOLD}Activating pre-push hook...{NC}")
    ...
    if not git_dir.is_dir():
        ...
        return 1
    # The committed hook must be executable for git to run it; ensure the bit
    # is set (a fresh checkout normally preserves it, but be defensive).
    source.chmod(source.stat().st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
    # Read the current setting first so the common already-armed case is a
    # genuine no-op (idempotent) rather than a redundant write.
    ...
    if current.returncode == 0 and current.stdout.strip() == ".githooks":
        cprint(f"  {GREEN}Already active: core.hooksPath = .githooks{NC}")
        return 0
    subprocess.run(["git", "config", "core.hooksPath", ".githooks"], cwd=str(root), check=False)
...
def run_gate(root: Path) -> int:
    ...
    # Gate 0.5: Pre-push hook integrity — the on-disk .githooks/pre-push must
    # match its committed sha256 pin (TRDD-71a2239a). READ-ONLY: an out-of-band
    # hook edit that wasn't re-pinned in the same commit is caught here, locally,
    # before the push — never silently weakening the hook for every clone. CI
    # re-runs the identical sha256 check post-push.
    cprint(f"\n{BLUE}[G0.5] Verifying pre-push hook integrity...{NC}")
    ok, detail = check_pre_push_hook_integrity(root)
    if not ok:
        cprint(f"  {RED}BLOCKED: {detail}{NC}")
        return 1
    cprint(f"  {GREEN}Pre-push hook matches its sha256 pin.{NC}")
    ...
    if not shutil.which("uvx"):
        ...
        return 1
    ve = subprocess.run(
        ["uvx", "--from", CPV_REF, "--with", "pyyaml", "cpv-remote-validate", "plugin", ".", "--strict"],
        cwd=str(root),
        timeout=600,
    ).returncode
    ...
    if not (root / ".jscpd.json").is_file():
        ...
        try:
            je = subprocess.run(["npx", "--yes", "jscpd", "."], cwd=str(root), timeout=600).returncode
        except (OSError, subprocess.SubprocessError) as exc:
    ...
    if not (test_dir.is_dir() and any(test_dir.glob("test_*.py"))):
        ...
        return 1
    try:
        # AIMAESTRO_CLI_REQUIRED=1 turns the frozen-CLI contract SKIPS into hard failures
        # (TRDD-YIOTS27H). Those tests check that every flag/subcommand a skill teaches
        # really exists, by reading each CLI's --help; with no ai-maestro installed they
        # skip, which is right for CI but wrong HERE: publish runs on a developer machine
        # where the CLIs exist, so a skip means the install broke -- and we would ship
        # skills whose taught flags were never verified, with the gate still green.
        test_env = {**os.environ, "AIMAESTRO_CLI_REQUIRED": "1"}
        te = subprocess.run(
            ["uv", "run", "pytest", "tests/", "-x", "-q", "--tb=short"], cwd=str(root), env=test_env, timeout=300
        ).returncode
    except subprocess.TimeoutExpired:
...
def stage_commit_and_push(root: Path, new_ver: str, dry_run: bool) -> None:
    ...
    # cannot be done.
    if not dep_tag:
        cprint(f"  {RED}✗ Cannot derive the dependency-resolution tag: no `name` in .claude-plugin/plugin.json.{NC}")
        raise SystemExit(1)
    if dep_tag_exists:
    ...
    if head_branch and head_branch != default_branch:
        cprint(
            f"  {RED}✗ Refusing to push: HEAD is on '{head_branch}', expected default branch '{default_branch}'.{NC}"
        )
...
def main() -> int:
    ...
    mode_group.add_argument(
        "--gate", action="store_true", help="Pre-push gate mode: lint + validate + tests only (no bump/push)"
    )
    mode_group.add_argument(
