 ...
 *
 * Supported events:
 * - Notification (idle_prompt): When Claude is waiting for user input — checks inbox
 * - Notification (permission_prompt): When Claude is waiting for permission approval
 * - Stop: When Claude finishes responding
 * - SessionStart: When a session starts/resumes — checks inbox after 3s delay
 *
...
async function main() {
    ...
    // NOTE: PermissionRequest handler moved to feature/permission-request-ui branch
    switch (hookEvent) {
        case 'Notification':
            ...
            if (notificationType === 'idle_prompt') {
                ...
                // Claude is waiting for permission approval (in terminal)
                writeState(cwd, {
                    status: 'waiting_for_input',
                    message: input.message || 'Waiting for permission approval...',
                    notificationType,
                    sessionId,
                    transcriptPath
                });
