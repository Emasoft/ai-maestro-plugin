...
**`manager`** (MANAGER), or **`user`** (USER). "Treat as non-exempt when
unsure" is the same rule as "escalate one rung when unsure". A TRDD records
the TITLE it needs in its `min-approval-requirement:` field.
