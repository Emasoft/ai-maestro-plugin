...
`trdd-approval-tiers`, `manager-approval-defaults`) live at their canonical
homes (janitor IND bases in `~/.claude/rules/`, ai-maestro DEP overlays in
each agent workdir's `.claude/rules/`). Re-pasting the
