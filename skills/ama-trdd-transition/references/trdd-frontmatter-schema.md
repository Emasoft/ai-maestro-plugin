...
| `min-approval-requirement` | enum | `none` | The TITLE that must approve this TRDD before it may execute: `none` \| `orchestrator` \| `chief-of-staff` \| `manager` \| `user` (a HIGHER rung may approve a lower one, never the reverse). `none` is agent-independent — authored directly in `design/tasks/`; any higher rung starts as a `proposal` in `design/proposals/`. Self-classified but audited against the objective requirement-floor — see the ai-maestro DEP overlay `.claude/rules/aimaestro-trdd-approval.md` §D3 (its single canonical home) and [approval-tiers-and-zones.md](approval-tiers-and-zones.md). |
| ~~`approval-tier`~~ | ~~int~~ | — | **RETIRED — never write it.** Legacy DECODE only: `0→none`, `1→chief-of-staff`, `2→manager`, `3→user`; rewrite the card to the title. `orchestrator` has **no** number — a 4-value numeric field structurally cannot express it, which is why the scheme was retired. `maestro` is a deprecated read-alias for `user`. |
