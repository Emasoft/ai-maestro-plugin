...
- **`docs-helper.sh`** - Sourced by the `docs-*.sh` tool scripts. Provides documentation-specific API functions (`docs_query`, `docs_index`) and initialization logic. Located alongside the tool scripts in `~/.local/bin/` (installed) or `scripts/ (in ai-maestro repo)` (source). If tool scripts fail with "common.sh not found", re-run the installer (`./install-doc-tools.sh`).
