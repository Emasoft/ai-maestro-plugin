...
All teams are closed (isolated messaging with COS gateway). <!-- Groups (was `/api/groups`) are PERMANENTLY out — MANAGER verdict ai-maestro#64: no CLI surface, no consumer, no design commitment. If lightweight collections are ever wanted, that is a NEW design, not a decoupling leftover. Do NOT call `/api/*` directly (core#11). -->
...
All teams are closed by design; there is no open/closed toggle to set. <!-- Change-team-type is PERMANENTLY out — MANAGER verdict ai-maestro#64: all teams are closed, the toggle is a no-op, no verb is pending. If a type ever returns, it returns with its own verb as a new design. -->
...
**This describes the AMP transport only.** Claude Code's native `SendMessage` / `ListAgents` reach another live session directly — on this machine, on any of your machines (2.1.224), a Remote Control session by name (2.1.225), or a cloud session (2.1.229) — with no server in the path that validates edges, so no edge is validated and no 403 is possible there. R6 binds the agent on both; only AMP can tell it when it broke R6. Since 2026-08-20 the point is moot for harness agents: a registry invariant (governance R42.9) writes `crossSessionInbound: "refuse"` into every registered agent workdir (re-applied on create/wake/sweep), so no native cross-session message can REACH one — AMP is the only inter-agent channel there. **One carve-out, since 2.1.248: a session launched with `--restricted` (or `CLAUDE_CODE_RESTRICTED=1`) IGNORES user, project and local settings files — including the `settings.local.json` the invariant writes that value into.** The invariant therefore does not bind such a receiver: its inbound posture is undefined by R42.9 rather than `refuse`. The sweep cannot close this, because the file is written correctly and simply not read. Treat "no message can reach a harness agent" as true of normally-launched agents only, and do not launch a registered agent workdir with `--restricted` while relying on the lockdown. Since 2.1.238 the sender learns this rather than getting a false success: the send reports `refused`. That signal is blanket, not edge-aware — it says the door is shut to everyone, never that this particular route broke R6 — so the sentence above stands unchanged: only AMP can tell an agent when it broke R6. Inbound-only: an outbound `permissions.deny: ["SendMessage"]` is forbidden (breaks subagent handling) and is removed by the same invariant. See the `agent-messaging` skill.
