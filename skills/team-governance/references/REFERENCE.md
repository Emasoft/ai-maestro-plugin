...
**This describes the AMP transport only.** Claude Code's native `SendMessage` / `ListAgents` reach another live session directly — on this machine, on any of your machines (2.1.224), a Remote Control session by name (2.1.225), or a cloud session (2.1.229) — with no server in the path that validates edges, so no edge is validated and no 403 is possible there. R6 binds the agent on both; only AMP can tell it when it broke R6. Since 2026-08-20 the point is moot for harness agents: a registry invariant (governance R42.9) writes `crossSessionInbound: "refuse"` into every registered agent workdir (re-applied on create/wake/sweep), so no native cross-session message can REACH one — AMP is the only inter-agent channel there. Inbound-only: an outbound `permissions.deny: ["SendMessage"]` is forbidden (breaks subagent handling) and is removed by the same invariant. See the `agent-messaging` skill.
