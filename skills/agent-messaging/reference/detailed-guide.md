...
## Communication Graph (Title-Based Directed Graph, R6 v3, 2026-05-04)
...
**Scope of that enforcement: AMP only.** Claude Code has a native session-to-session transport (`SendMessage` / `ListAgents`) that never reaches the ai-maestro server, so `validateMessageRoute()` is not consulted and a forbidden edge over it returns no error. The graph still binds the agent there; and since 2026-08-20 (USER directive; governance R42.9) the channel is CLOSED INBOUND inside the harness — a registry invariant writes `crossSessionInbound: "refuse"` into every registered agent workdir's `.claude/settings.local.json` (re-applied on create/wake/sweep), so a normally-launched harness agent cannot receive a native cross-session message. **One carve-out, since 2.1.248: a session launched with `--restricted` (or `CLAUDE_CODE_RESTRICTED=1`) IGNORES user, project and local settings files — including the `settings.local.json` the invariant writes into.** Such a receiver is therefore not bound: its inbound posture is undefined by R42.9 rather than `refuse`, and the sweep cannot close it because the file is written correctly and simply never read. Do not launch a registered agent workdir with `--restricted` while relying on the lockdown. Filed upstream as `Emasoft/ai-maestro#154`. Since 2.1.238 the SENDER is told so: such a send reports `refused` instead of a silent success. Since 2.1.248 a MALFORMED value fails loudly too, and asymmetrically by scope — warn-and-HOLD in user settings, REFUSE in managed settings — so a typo written by the invariant no longer degrades to an open inbox. Read that narrowly — the refusal is blanket, not edge-aware. It fires for every sender alike, so it reports a shut door and never which edge R6 forbids; it is not graph enforcement arriving over the native path. Inbound-only by design: an outbound `permissions.deny: ["SendMessage"]` is forbidden (it breaks subagent handling) and the invariant removes it. Use `amp-send.sh` for inter-agent coordination, and see the SKILL body's *"The 403 covers AMP"* section.
...
### Adjacency Matrix (v3, 2026-05-04)
...
directives directly to team agents. The **CHIEF-OF-STAFF is now the SOLE
inbound and outbound gateway for closed-team agents**. MANAGER still
freely reaches COS, peer MANAGERs, MAINTAINER, AUTONOMOUS, and HUMAN.
...
| **R6.2** | MANAGER has `Y` to COS + peer MANAGER + governance layer (MAINTAINER, AUTONOMOUS) + HUMAN. **In-team non-COS titles (ORCH, ARCH, INT, MEM) are blank for MANAGER as of v3 (2026-05-04)** — MANAGER must route through COS. |
...
| **R6.11–R6.14** | Canonical agent address format (2026-05-06): single ID per host, wire form `<agent-id>@<host>` or `<host>:<agent-id>`, bare `<agent-id>` resolves to writer's host. Persona name may alias the agent-id when no collision exists; collisions return HTTP 409 `disambiguation_required`. The legacy 3-level hierarchical (`team/sub/name`) format is deprecated — see the bundled GOVERNANCE-RULES.md for the full specification. |
