...
description: "Discover MCP server tools, resources, and prompts without installing plugins. Use when exploring MCP servers or debugging connections. Trigger with 'what tools does this MCP server expose', 'explore an MCP server without installing it', 'my MCP connection isn't working'. Loaded by ai-maestro-plugin"
