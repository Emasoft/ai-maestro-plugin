...
severity: HIGH
relevant-rules: []
implementation-commits: [f6c6d6d]
---
...
**Status:** Complete — shipped in f6c6d6d (pending push). Layer-1 interpreter-match + nonce + defence-in-depth log landed in the static `.githooks/pre-push` (+ T0 reconciled the false header comment); tests in `tests/test_pre_push_gate.py`. Verified fix-plan: `reports/go-on-yourself/20260622_102243+0200-security-trio-plan.md`.
**Filed:** 2026-05-08

> **Implementation note (2026-06-24, shipped `f6c6d6d`):** the Layer-2 / Dependencies
> framing below — a nonce added to `PRE_PUSH_HOOK_TEMPLATE` inside publish.py — was
> VERIFIED STALE on 2026-06-22: that template and `ensure_pre_push_hook` were deleted in
> commit `2073681` (CPV-pipeline migration), so the hook is now a **static committed file**
> (`.githooks/pre-push`). The hardening landed directly in that static hook, and
> prerequisite T0 reconciled the false "auto-generated by publish.py" header comment. The
> original design prose below is preserved as the filing; see the verified fix-plan for the
> as-shipped shape.
